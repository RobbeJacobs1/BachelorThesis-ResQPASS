% This function sovle a specific contact problem. 
function [] = solve_problem(filename, lb_x, ub_x, lb_y, ub_y, lb_z, ub_z, preconditioning)
    % Initialise values
    if nargin < 8
        preconditioning = false;
    end

    filepath = "Create_3D_meshes/Parsed_meshes/" + filename + ".mat";
    
    % Load files
    load(filepath, "nodes", "triangles", "outer_faces");
    ...
    lb = [lb_x*ones(nV, 1) lb_y*ones(nV, 1) lb_z*ones(nV, 1)] - nodes;
    ub = [ub_x*ones(nV, 1) ub_y*ones(nV, 1) ub_z*ones(nV, 1)] - nodes;
    ...
    [sol, history] = ResQPASS_altered(R, zeros(Ndof, 1), lb, ub, ...
        "M1", M1, "displayResidual", true);                  % Solve the contact problem
    
    % Update the points with the optimal differences
    sol_nodes = nodes + reshape(sol, 3, nV)';

    if preconditioning
        filename = filename + "_precond";
    end

    save("Create_3D_meshes/Results/" + filename + ".mat", "nodes", ...
        "triangles", "sol_nodes","sol", "history", "Q", "R", ...
        "lb_x", "ub_x", "lb_y", "ub_y", "lb_z", "ub_z", "outer_faces");
end